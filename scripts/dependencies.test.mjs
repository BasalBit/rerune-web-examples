...
const lock = { packages: Object.fromEntries(names.map(name => [`@rerune/${name}@1.4.0`, { resolution: { integrity: 'sha512-test' } }])) }
const manifest = { private: true, dependencies: { '@rerune/react': '1.4.0' } }

test('rejects workspace links and unpublished candidate pins', () => {
  for (const pin of ['workspace:*', 'file:../sdk.tgz', '1.3.0-rc.1', '^1.4.0']) {
...
test('rejects mixed transitive SDK versions and private lockfile paths', () => {
  ...
  assert.equal(validateConsumer([manifest], lock), '1.4.0')
})
